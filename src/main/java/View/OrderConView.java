...
public class OrderConView extends JDialog {
	...
	public static void main(String[] args) {
		try {
			OrderConView dialog = new OrderConView(123, new String[] { "John Doe", "123456789", "123 Street" }); // Giả
																													// //
																													// hàng
	 ...
	 * Create the dialog.
	 */
	public OrderConView(int orderId, String[] customerInfo) {
		this.customerInfo = customerInfo; // Lưu thông tin khách hàng/nhà sản xuất
			...
			buttonPane.setLayout(null);
			{
				...
				okButton.addActionListener(e -> {
					...
					for (int i = 0; i < rowCount; i++) {
						...
						int rowsAffected = orderDetailsDAO.insert(orderDetailsModel); // Thêm chi tiết đơn hàng vào cơ
																						// sở dữ liệu
						if (rowsAffected > 0) {
							rowData.add(new Object[] { orderDetailsModel.getProductid(), model.getValueAt(i, 1),
									orderDetailsModel.getQuantity(), orderDetailsModel.getUnitprice(),
									orderDetailsModel.getUnitprice() });
						}
					}
					if (rowData.size() > 0) {
						try {
							PDF.createSalesInvoice(orderId, rowData, customerInfo[0], customerInfo[1], customerInfo[2]);
							JOptionPane.showMessageDialog(this, "Đơn hàng đã được tạo", "Thông báo",
									JOptionPane.INFORMATION_MESSAGE);
							OrderView orderView = new OrderView();
							orderView.updateTotalLabel(orderId);
